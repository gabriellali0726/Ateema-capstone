# app/streamlit_app_1.py
# Ateema – Proposal Builder (old layout restored + dual input modes)
...
with st.sidebar:
    ...
    # Kept for parity with old sidebar; not used by deterministic allocator
...
def qty_from_tier(tier: str) -> int:
    """Treat tiers like '3X' as a quantity; otherwise 1."""
    if not tier:
        return 1
...
def make_reasoning(product_name: str, option: str, tier: str, pool_label: str,
    ...
    # Short, deterministic 2–3 sentences grounded in user inputs.
    bits = []
...
def rows_from_selection(label: str,
                        sel,
                        focus: str,
                        market_target: str) -> List[Dict]:
    out = []
...
focus_text = ""
market_text = ""
is_advertiser = True            # default; used by Summit booth rule
...
if input_mode == "Load JSON":
    ...
    if st.button("Generate Proposal", type="primary"):
        ...
        # Expect the structure you’ve been using
        profile = raw.get("client_profile", "")
        total_budget = float(raw.get("budget", 0))
        similar_clients = raw.get("similar_clients", [])
        chosen = list(raw.get("candidate_products", []))  # keep as given
        # Extract focus/target for reasoning if present in the profile string
    ...
    # Tourist slider with larger live number; remove big Industry metric
    ...
    # Candidate products – start EMPTY on reload
    ...
    if st.button("Generate Proposal", type="primary"):
        # parse similar clients for completeness (not used in allocator)
        parsed = []
        for line in (sc_text or "").splitlines():
            ...
            if not parts or not parts[0]:
                continue
            name = parts[0]
            purchased = [p.strip() for p in (parts[1].split(",") if len(parts) > 1 else []) if p.strip()]
            notes = parts[2] if len(parts) > 2 else ""
            parsed.append({"name": name, "purchased": purchased, "notes": notes})
        st.session_state["_payload_similar"] = parsed
        st.session_state["_trigger_generate"] = True
...
if st.session_state.get("_trigger_generate"):
    ...
    # Partition by category and allocate
    ...
    # ---------- Tourist then Industry (stacked, not parallel) ----------
    def _render_table(label: str, sel):
        ...
        if rows:
            ...
            df = pd.DataFrame(
                rows,
                columns=["product", "option", "qty", "unit_price", "total_price", "reasoning"],
            )
